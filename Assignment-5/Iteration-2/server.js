...
io.on('connection', (socket) => {
	...
	uploader.on('complete', (fileInfo) => {
		...
		detectFaces('./data/' + fileInfo.name,function (info) {
          ...
          var locations = getFaceLocations(info);
		  ...
		  socket.emit("done",({total: totalFaces, ages: avgAges, gender: genders, location: locations}));
...
/*
  ...
  Description: get the face locations of each face
  Param: faceInfo - information about faces
  Precondition: information should be valid
  Postcondition: face location of each face found
  return: face locations


*/

function getFaceLocations(faceInfo) {
    
    logger.info('Start getFaceLocations Function');
    var allfaces = faceInfo.images[0].faces
	var locations = []
	logger.verbose('Loop Start'); 
    for (var i = 0; i < allfaces.length; i++) {
		  var face = allfaces[i];
          var x = face.face_location.left;
          var y = face.face_location.top;
          var height = face.face_location.height;
          var width = face.face_location.width;
		  var faceNo = i + 1
		  locations.push([x,y,width,height]);
		  logger.debug('Iteration' + i + ' Face-' + faceNo + ' x-position: ' + x + ' y-position: ' + y + ' height: ' + height + ' width: ' + width);
	}
	logger.verbose('Loop End');
	logger.info('End of getFaceLocations Function');
	return locations;
